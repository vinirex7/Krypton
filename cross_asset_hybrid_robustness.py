"""Robustness-only validation for the frozen cross-asset hybrid.

This file does not tune the strategy. It fixes the already successful v2 design
at 10% alpha and tests whether the result survives nearby allocator cadences
(21/30/45 days). Promotion requires all cadences to pass the same frozen gates.
"""
...
def run(start, end, mc_runs=5000):
    ...
    mc = {name: dv.block_bootstrap_monte_carlo(
        curve.pct_change().dropna(), runs=mc_runs, seed=42)
        for name, curve in curves.items()}

    full.to_csv("cross_asset_hybrid_robustness_full.csv", index=False)
    periods.to_csv("cross_asset_hybrid_robustness_periods.csv", index=False)
    pd.DataFrame(curves).sort_index().ffill().to_csv("cross_asset_hybrid_robustness_curves.csv")
    (pd.concat(logs, ignore_index=True) if logs else pd.DataFrame()).to_csv(
        "cross_asset_hybrid_robustness_rebalances.csv", index=False)
    report = {
